...
test('500 MB accepted, larger files rejected', () => {
  assert.equal(validateUpload(request).chunks, 100);
...
test('exact chunk sizes and final remainder', () => {
  assert.equal(expectedChunkSize(MAX_FILE_BYTES,99),CHUNK_BYTES);
  assert.equal(expectedChunkSize(CHUNK_BYTES+3,1),3);
  for (const index of [-1,100,0.5]) assert.throws(()=>expectedChunkSize(MAX_FILE_BYTES,index));
});
